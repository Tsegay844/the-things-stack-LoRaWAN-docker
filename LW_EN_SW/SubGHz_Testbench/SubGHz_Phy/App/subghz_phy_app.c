...
// RX CYCLE TIME CONFIGURATION (values are in miliseconds)
...
/** Calculated Cycle time for each Data Rate configuration **/
#define RX_CYCLE_TIME_DR0	5*(TEST_N_PKTS*(TOA_DR0_16BYTES+TEST_TX_PKT_INTERVAL_MS))+1000
#define RX_CYCLE_TIME_DR1	5*(TEST_N_PKTS*(TOA_DR1_16BYTES+TEST_TX_PKT_INTERVAL_MS))+1000
#define RX_CYCLE_TIME_DR2	5*(TEST_N_PKTS*(TOA_DR2_16BYTES+TEST_TX_PKT_INTERVAL_MS))+1000
#define RX_CYCLE_TIME_DR3	5*(TEST_N_PKTS*(TOA_DR3_16BYTES+TEST_TX_PKT_INTERVAL_MS))+1000
#define RX_CYCLE_TIME_DR4	5*(TEST_N_PKTS*(TOA_DR4_16BYTES+TEST_TX_PKT_INTERVAL_MS))+1000
#define RX_CYCLE_TIME_DR5	5*(TEST_N_PKTS*(TOA_DR5_16BYTES+TEST_TX_PKT_INTERVAL_MS))+1000
#define RX_CYCLE_TIME_DR6	5*(TEST_N_PKTS*(TOA_DR6_16BYTES+TEST_TX_PKT_INTERVAL_MS))+1000
...
/* USER CODE BEGIN PV */
// TESTBENCH CONFIG
uint8_t tx_power_dbm = DEFAULT_TX_OUTPUT_POWER; // TB initial TX Power value
uint8_t lora_data_rate = DEFAULT_DATA_RATE + DATA_RATE_OFFSET; // TB initial DR value
int synch_retransmit_ctr = 0; // In RX Mode, counts the number of attempts during the synch process
...
uint32_t tx_start_timestamp = 0; // RTC count to estimate the TOA
uint32_t cycle_start_timestamp = 0; // RTC count to estimate the total cycle time
...
static void Tb_OnRxDone(uint8_t *payload, uint16_t size, int16_t rssi, int8_t LoraSnr_FskCfo)
{
	...
	/* Record payload content, RSSI and SNR values*/
	...
	switch (Testbench_State){
		...
		case TB_TX_DONE:
			...
			if (tx_synch_flag == true){ // done transmitting REPLY synch frame
				...
				n_tx_ctr = 0;
				// Configure the Radio for the Next DR Cycle based on the synch. frame data
